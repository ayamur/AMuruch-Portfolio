function About() {

  return (

    <>
      <h1>About Ayala Murúch:</h1>
      <div class='about-all'>
        <div class='about'>
          <h2>Software Engineer with a Background in Design, Non-Profits, and Game Testing. Passionate about Coding, Cryptids, and Uplifting Others! </h2>
          <p>I am currently working through General Assembly’s Software Engineer Full-Time Immersion Program. This program is fast-paced and intense, but participating is really allowing me to hone in on my strengths and use them to gain a variety of new skills and growth in a supportive environment.
          </p>
          <p>
            I have always been passionate about uplifting the moods and spirits of others when possible, with my strong empathy and compassion. While I am creative and enthusiastic, this program is allowing me to pursue other avenues of thinking and to process information with a more logical approach. Most recently, I learned a valuable lesson about the importance of celebrating small victories, personal and professional, in the midst of larger endeavors.
          </p>
          <p>
            My past is varied and has allowed me the opportunity to be flexible and grow in a variety of situations, from teaching English overseas to all age groups, to learning what it takes to develop and maintain a small local nonprofit as a PR and graphics assistant. Even while I have focused on adapting and growing in my personal life, that does not stop me from finding ways to do what I enjoy by helping others when the opportunity arises.
          </p>
          <p>
            My biggest goal is to continue to pursue my career path in the field of tech that will allow me to continue to pursue and understand different languages, but at the same time be a place where I can really help others, including those I work with, with my strengths of multiple soft-skills and genuine care and concern for others.
          </p>
          <h3>
            Backgrounds: Graphic Arts and Design, Non-Profit Sector, Professional and Personal Deep Research and Verification, Project Planning, Event Coordination, Costuming, Promotional Marketing Design, QA Game Testing, and Overseas Language Education for Adults and Children.
          </h3>
          <p>
            Hobbies include:
            Gaming, Digital Art Creation, Dancing, Reading, Hands On Arts and Crafts, Anything Paranormal /Supernatural, Learning and Researching, Travel and Culture, Designing, Cosplay, Awkwardly Trying New Things, Completing Personal Growth Goals, and Daydreaming/Brainstorming!</p>
        </div>
      </div>
      <a href='#top' class='up' alt='jellyfish and arrow'>
        <img src='/jelly_up.png' alt='jellyfish with up arrow' class='jellyarrow' />
      </a>
    </>

  )

}

export default About